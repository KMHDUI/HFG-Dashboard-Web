...
export default function SignUp() {
  ...
  function mesageInformationSuccess() {
    return (
      <div className="flex flex-col justify-center">
        ...
        ></Image>
        {invalidInput ? (
          <p>Invalid input or user already exists.</p>
        ) : (
          <p className="mt-10">Akun anda berhasil didaftarkan</p>
        )}
        <Button
          name={"close"}
          className={""}
      ...
      <div className="fixed top-0 left-0 right-0 z-50 p-4 w-screen h-screen flex justify-center items-center">
        <div className=" relative bg-white drop-shadow-md rounded-xl overflow-hidden p-10">
          {loading ? (
            ...
            success ? (
              mesageInformationSuccess()
            ) : (
              messageInformationFailed()
            )
          ) : (
            <></>
          )}
  ...
  function HandleSubmitForm() {
    if (
      ...
      setLoading(true);
      axios
        .post("https://api-hfg-3s5y7jj3ma-as.a.run.app/api/v1/user/register", {
      ...
      setInvalidInput(!invalidInput);
      setModal(true);
      setMessageFailed("Data tidak lengkap atau password tidak sesuai");
    }
  }
        ...
        <section className="hidden md:block overflow-hidden relative h-full rounded-3xl">
          <div>
            <Image
              src={Cover}
              alt="cover"
              className="absolute z-0 md:scale-[3] lg:scale-[2]"
            />
          ...
          <div className="grid lg:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <Input
                typeInput="text"
                className=""
                onChange={setAsalPendididkan}
              />
            </div>
          </div>
          <input
            type="submit"
            className="cursor-pointer mt-14 mx-16 xl:mx-28 bg-[#064C72] p-2 rounded-full text-white"
            onClick={(e) => {
              e.preventDefault();
              HandleSubmitForm();
            }}
            name="Sign Up"
          />
          <span className="flex gap-1 justify-center w-full text-[#064C72] mb-5">

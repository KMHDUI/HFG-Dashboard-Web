...
export default function Home() {
  ...
  const [meCompetition, setMeCompetititon] =
    useState<listMeCompetitionType[]>();
    ...
    return (
      <div className="rounded-2xl border-[#ED6335] border-2 overflow-hidden">
        <p className="text-center p-2">
          Anda saat ini belum dapat mendaftar kompetisi yang tersedia. Harap
          melakukan verifikasi sebagai peserta. Jika anda telah melakukan
          verifikasi, harap menunggu admin untuk verifikasi akun anda.
        </p>

        <Link href={"/dashboard/verifikasi"}>
          <button className="w-full bg-[#ED6335] p-2 mt-4">
            Verifikasi Akun
          </button>
        </Link>
      </div>
  ...
  const choseFutsal = () => {
    return (
      <div className="bg-[#CA2812] fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
        helo
      </div>
    );
  };
  ...
  useEffect(() => {
    ...
    axios
      .get(
        "https://api-hfg-3s5y7jj3ma-as.a.run.app/api/v1/competition/me",
        config
      )
      .then((ress) => {
    ...
    axios
      .get("https://api-hfg-3s5y7jj3ma-as.a.run.app/api/v1/competition")
      .then((response) => {
  ...
  const cardCompetition = (
    ...
    return (
      <div
        className="min-h-[120px] w-full md:min-w-[350px] bg-white  rounded-2xl p-10"
        id={`${id}`}
      >
        ...
        <div className="flex flex-col md:flex-row gap-2">
          <button
            className="border-2 border-[#064C72] text-[#064C72] w-full p-2 rounded-2xl font-semibold"
            onClick={() => {
              if (type == "Essay") {
                router.push(`/esai`);
          ...
          type.toLocaleLowerCase() == "futsal" ? (
            <button
              className="bg-[#064C72]  w-full p-2 rounded-2xl text-white opacity-75"
              disabled
            >
              Register
            </button>
            ...
            <button
              className="bg-[#064C72] w-full p-2 rounded-2xl text-white"
              onClick={() => {
                handlerRegister(id);
              }}
            >
      ...
      >
        <div>
          ...
          {paymentStatus == "Pending" ? (
            <p
              className={`text-sm bg-[#FFB21E] rounded-full p-1 text-center min-w-[110px]`}
            >
              {paymentStatus}
            </p>
          ) : (
            <p
              className={`text-sm bg-[#FF1A1A] rounded-full p-1 text-center min-w-[110px]`}
            >
              {paymentStatus}
            </p>
          )}
  ...
  function handlerRegister(id: String) {
    ...
    axios
      .post(
        "https://api-hfg-3s5y7jj3ma-as.a.run.app/api/v1/competition/register",
        {
          competitionId: id,
        },
        config
      ...
      <div className="min-h-screen w-full bg-[#F6F8FD]">
        <section className="p-5 px-16 flex justify-end">
          <button
            ...
            className="p-2 bg-[#CA2812] rounded-full p-2 px-4 flex gap-4"
          >
        ...
        <section className="flex flex-col md:grid md:grid-cols-2 gap-3 md:flex-row p-8 md:p-20">
          <div className="">
            ...
            <div className="mt-20 md:max-w-[90%] drop-shadow-lg	rounded-3xl bg-white p-6 overflow-auto md:overflow-none">
              <TableContainer>

...
export default function Verifikasi() {
    ...
    const [selectedFilesHaloBelanja, setSelectedFilesHaloBelanja] = useState<File | null>(null);
    ...
    const handleVerifikasi = async () =>{
        ...
        if(selectedFilesKTM!=null && selectedFilesHaloBelanja!= null &&
            angkatan!= null && tanggalLahir!= null 
            ) {
            const linkKTM = await uploudFile(selectedFilesKTM, 'KTM');
            const linkHaloBelanja = await uploudFile(selectedFilesHaloBelanja, 'AppHaloBelanja')
            setDone(!done)
            if(linkKTM.success == true && linkHaloBelanja.success == true){
                axios.post('https://api-hfg-3s5y7jj3ma-as.a.run.app/api/v1/user/verification', {
                    major: jurusan,
    ...
    function loadingAnimation(){
        ...
        {done ? <div></div> : <div className=" relative bg-white drop-shadow-md rounded-xl overflow-hidden p-10">
            ...
            <form className='flex flex-col gap-3 p-10 md:p-20 w-full'>
                ...
                <div className='w-full grid grid-cols-1 md:grid-cols-2 gap-10 mt-10'>
                    <div>
                        <div className='mt-4'>
                            <p>Uploud File KTM</p>
                            <DropInput selectedFiles={selectedFilesKTM}  setSelectedFiles={setSelectedFilesKTM} ></DropInput>
                        </div>
                    </div>

                    <div>
                            <div className='mt-4'>
                                <p>Uploud File Halo Belanja</p>
                                <DropInput selectedFiles={selectedFilesHaloBelanja}  setSelectedFiles={setSelectedFilesHaloBelanja} ></DropInput>
                            </div>
                    </div>
                </div>
